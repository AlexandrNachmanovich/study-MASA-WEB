...
// BREAK –
// we ask a person to input 5 actor names.
// Add them to the array "actors"
//If input is empty we stop.
// chek if the user enter 5 actor names

const actors = [];
let actor;

for (let i = 0; i < 5; i += 1) {
  actor = prompt("Enter actor name");
  if (actor) {
    actors.push(actor);
  } else {
    break;
  ...
  if (actors.length < 5) {
    alert("Realy dude? You could not enter 5 actor names???");
  }
}
